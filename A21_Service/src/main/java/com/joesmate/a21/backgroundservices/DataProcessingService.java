        ...
        switch (data[0]) {
            ...
            case (byte) 0x32: {
                switch (data[1]) {
                    case (byte) 0x50: {
                        try {
                            App.getInstance().tts.speak(getString(R.string.PleaseDropIdCard));
                    ...
                    case (byte) 0x62: {//开始刷卡
                        App.getInstance().tts.speak(getString(R.string.PleaseBrushMagneticCard));
                        ToolFun.Dalpey(1000);
            ...
            case (byte) 0xC6: {
                switch (data[1]) {
                    ...
                    case (byte) 0x02: {//指纹
                        switch (data[2]) {
                            case (byte) 0x00: {//透传
                                App.getInstance().tts.speak(getString(R.string.PleasePushFp));
            ...
            case (byte) 0xc7: {
                switch (data[1]) {
                    case (byte) 0x01: {//签名
                        try {
                            App.getInstance().tts.speak(getString(R.string.PleaseSign));
            ...
            case (byte) 0x50: {//银行卡
                switch (data[1]) {
                    case (byte) 0x04: {//pboc
                        App.getInstance().tts.speak(getString(R.string.PleaseBankCard));
                        switch (data[2]) {
                            case (byte) 0x00: {
...
//            @Override
//            public void run() {
//                byte[] out = "收到/n".getBytes();
//                libserialport_api.device_write(App.getInstance().m_btfd, out, out.length);
//            }
//        }.start();
    ...
    protected void SendData(byte[] writedata, int writeLen) {
        int len = writeLen;
    ...
    protected void SendReturnData(byte[] writedata, int writeLen) {
        int len = writeLen + 2;
        byte[] Writebuffer = new byte[len + 5];
        Writebuffer[0] = 2;
        Writebuffer[1] = (byte) (len >> 8);
