...
/**
 * Created by andre on 2017/5/26 .
 */

public class Funstion {
    public static boolean GetPermission(String Path) {
        File fl = new File(Path);
        try {
                /* Missing read/write permission, trying to chmod the file */
            Process su;
            su = Runtime.getRuntime().exec("sh");
            String cmd = "su \n" + "chmod -R 666 " + fl.getAbsolutePath() + "\n" + "exit \n";
    ...
    public static String ProcessingData2String(byte[] data, int dRead) throws Exception {
        String str = "";
        if (data[0] == 0x02) {
            byte[] dt = new byte[dRead - 2];
            System.arraycopy(data, 1, dt, 0, dRead - 2);

            str = new String(dt, "GBK");//ToolFun.printHexString(dt); //
        } else if (data[1] == (byte) 0xFF && data[2] == (byte) 0x55) {
            if (data[5] == (byte) 0x60 && data[6] == (byte) 0xB3) {
                int len = (data[12] & 0xffffffff) + (data[13] << 8) & 0xffffffff;
                byte[] tmp = new byte[len];
                System.arraycopy(data, 14, tmp, 0, len);
                str = byte2HexStr(tmp, len);//new String(tmp, "GBK");
            } else if (data[5] == (byte) 0x70) {
                int len = (data[3] & 0xffffffff) + (data[4] << 8) & 0xffffffff;
                byte[] tmp = new byte[len];
                System.arraycopy(data, 6, tmp, 0, len);
